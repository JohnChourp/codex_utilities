...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Audit DynamoDB TableName/IndexName usage across CRP lambdas.",
    )
    parser.add_argument("--root", default=DEFAULT_ROOT, help=f"Lambdas root (default: {DEFAULT_ROOT})")
    parser.add_argument(
        "--mode",
...
def build_report(
    ...
    for usage in usages:
        evidence = Evidence(usage.lambda_name, usage.file_path, usage.line)
        ...
        unresolved.append(
            {
                ...
                "reason": "; ".join(part for part in [usage.table_reason, usage.index_reason] if part),
            }
        )
        ...
        "resolved_table_index_pairs": [
            {
                ...
                "index": index,
                "evidence": [
                    {"lambda": ev.lambda_name, "file_path": ev.file_path, "line": ev.line}
    ...
    for (table, index), evidences in sorted(resolved_pairs.items()):
        ...
        target = {"table": table, "index": index, "evidence": rendered_evidence}
...
def render_text(report: dict) -> str:
    ...
    if report["missing_in_dynamodb"]:
        ...
        for item in report["missing_in_dynamodb"]:
            evidence = ", ".join(f"{ev['file_path']}:{ev['line']}" for ev in item["evidence"][:3])
