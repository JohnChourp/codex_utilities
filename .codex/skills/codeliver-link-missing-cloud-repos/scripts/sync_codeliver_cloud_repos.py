#!/usr/bin/env python3
"""Sync Codeliver project cloud repos for GitHub repositories that contain a target token."""
...
def _build_endpoints(route_prefix: str) -> dict[str, str]:
    prefix = (route_prefix or DEFAULT_API_ROUTE_PREFIX).strip()
    return {
        "renew": f"{API_BASE}/{prefix}-renew-token",
        "fetch_projects": f"{API_BASE}/{prefix}-fetch-projects",
        "fetch_features": f"{API_BASE}/{prefix}-fetch-features",
        "fetch_subprojects": f"{API_BASE}/{prefix}-fetch-subprojects",
        "fetch_cloud_repos": f"{API_BASE}/{prefix}-fetch-cloud-repos",
        "handle_subproject": f"{API_BASE}/{prefix}-handle-subproject",
        "handle_feature": f"{API_BASE}/{prefix}-handle-feature",
        "handle_cloud_repo": f"{API_BASE}/{prefix}-handle-cloud-repo",
    }


ENDPOINTS = _build_endpoints(DEFAULT_API_ROUTE_PREFIX)
...
def _select_project(cfg: dict[str, Any], token: str, project_name: str, project_id: str) -> tuple[str, str]:
    ...
    if not isinstance(rows, list):
        raise RuntimeError("Unexpected response from fetch-projects endpoint")
...
def _fetch_feature_cloud_repos(project_id: str, feature_id: str, token: str) -> list[dict[str, Any]]:
    res = _http_post(
        ENDPOINTS["fetch_cloud_repos"],
        {
            "type": "fetch-cloud-repos-by-feature-id",
...
def _collect_cloud_repos_from_all_features(
    project_id: str,
    project_features: list[FeatureRef],
    token: str,
    project_cloud_rows: list[dict[str, Any]],
) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    repo_ids_by_feature: dict[str, list[str]] = {}
    feature_errors: list[dict[str, str]] = []
    feature_calls_ok = 0
    merged_repo_ids: set[str] = set()

    for feature in project_features:
        try:
            rows = _fetch_feature_cloud_repos(project_id, feature.feature_id, token)
            feature_calls_ok += 1
            ids = sorted(
                {
                    str(row.get("repo_id") or "").strip()
                    for row in rows
                    if isinstance(row, dict) and str(row.get("repo_id") or "").strip()
                }
            )
            repo_ids_by_feature[feature.feature_id] = ids
            merged_repo_ids.update(ids)
    ...
    source = "all-features"
    if not merged_repo_ids:
        fallback_ids = sorted(
            {
                str(row.get("repo_id") or "").strip()
                for row in project_cloud_rows
                if isinstance(row, dict) and str(row.get("repo_id") or "").strip()
            }
        )
        merged_repo_ids.update(fallback_ids)
        source = "project-fallback"

    feature_name_map = {feature.feature_id: feature.feature_name for feature in project_features}
    feature_rows: list[dict[str, Any]] = []
    for feature in project_features:
        repos = repo_ids_by_feature.get(feature.feature_id, [])
        feature_rows.append(
            {
                "feature_id": feature.feature_id,
                "feature_name": feature_name_map.get(feature.feature_id, feature.feature_name),
    ...
    summary = {
        "search_mode": "all-features",
        "source": source,
        "features_scanned": len(project_features),
        "features_fetch_success_count": feature_calls_ok,
        "features_fetch_error_count": len(feature_errors),
        "feature_errors": feature_errors,
        "cloud_repo_count": len(merged_repo_ids),
        "cloud_repos": sorted(merged_repo_ids),
        "features": feature_rows,
    }
    return summary, feature_errors
...
def _create_subproject(project_id: str, token: str, subproject_name: str) -> dict[str, Any]:
    body = {
        ...
        "subProject": {
            ...
            "project_id": project_id,
            "res_aws": {},
            "description": "Auto-created for Codeliver cloud repo auto-linking.",
        },
    }
...
def _create_feature(project_id: str, subproject_id: str, token: str, feature_name: str) -> dict[str, Any]:
    body = {
        ...
        "feature": {
            ...
            "feature_name": feature_name,
            "res_aws": {},
            "description": "Auto-created feature for Codeliver cloud repo auto-linking.",
        },
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--all-cloud-repos-out", default=str(DEFAULT_ALL_CLOUD_REPOS_OUT))
...
def main() -> int:
    ...
    project_features = _fetch_features(project_id, token)
    cloud_rows = _fetch_project_cloud_repos(project_id, token)
    all_feature_cloud_summary, _ = _collect_cloud_repos_from_all_features(
        project_id=project_id,
        project_features=project_features,
        token=token,
        project_cloud_rows=cloud_rows,
    )

    all_cloud_repos_out = Path(args.all_cloud_repos_out).expanduser().resolve()
    _write_json(
        all_cloud_repos_out,
        {
    ...
    report = {
        ...
        "project_feature_count": len(project_features),
        "all_cloud_repos_out": str(all_cloud_repos_out),
        "all_feature_cloud_repos": all_feature_cloud_summary,
